...
#     def __init__(self, env, max_opponents=5):
#         super().__init__(env)
#         self.max_opponents = max_opponents
#         self.opponent_pool = deque(maxlen=max_opponents)
#         self.current_opponent = None
        
#     def add_opponent(self, checkpoint_path, global_step):
#         print(f"Adding opponent (Step {global_step}): {checkpoint_path}")

#         if len(self.opponent_pool) >= self.max_opponents:
#             self.opponent_pool.popleft()
...
#         # 默认使用最新添加的对手
#         self.current_opponent = self._create_opponent(state_dict)
...
if __name__ == "__main__":
    ...
    run_name = f"0826_1659_cuda0_normal_cnn_self_play_step_penalty_two_agent_switch_map_decay_{args.env_id}__{args.exp_name}__{args.seed}__{int(time.time())}"
